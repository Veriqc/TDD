...
class TDD:
    def __init__(self,node,weight=cn1):
        """TDD"""
        self.weight = weight
#         print(weight,id(weight),id(cn0),id(cn1))
        self.index_set=[]
...
def Ini_TDD(index_order=[],max_rank=200):
    ...
    terminal_node.is_key_node=True
...
#     if tdd1.node.is_key_node and tdd2.node.is_key_node and the_key!='c':
...
#         return tdd
    ...
    if cont_order[0][k1]<cont_order[1][k2]:
        ...
        if the_key!='c':
            ...
            if tdd1.node.is_key_node and tdd2.node.is_key_node:
                tdd = find_computed_table(['*',tdd1,tdd2,temp_key_2_new_key,cont_num])
                store = True
            if tdd:
                tdd1.weight=w1
...
#     if key_2_new_key[0][k1]!='c' or key_2_new_key[1][k2]!='c':
#         tdd.node.is_key_node=True
#     if key_2_new_key[0][k1]==k1 or key_2_new_key[0][k1]==key_2_new_key[1][k2]:
#     if len(temp_key_2_new_key[0])%2==0 and len(temp_key_2_new_key[1])%2==0: 
    if store:
        tdd.node.is_key_node=True
...
def Slicing(tdd,x,c):
    ...
    if k==x:
        res=TDD(tdd.node.successor[c],tdd.node.out_weight[c])
        return res
    else:
